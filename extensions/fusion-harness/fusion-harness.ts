...
export default function (pi: ExtensionAPI) {
	...
	pi.registerFlag("fh-config", {
		type: "string",
		description: "Stack YAML path. OMP uses ~/.omp/agent/fusion-harness/model-stack-trio.yaml when present if omitted.",
	});
	...
	const ensureConfigLoaded = () => {
		...
		configLoaded = true;
		const configPath = flagStr("fh-config") || rawCliFlag("fh-config") || process.env.FH_CONFIG?.trim() || (isOmpRuntime() ? findGlobalStackConfig() : undefined);
	...
	pi.registerCommand("fh-system-prompt", {
		...
		handler: async (_args, ctx) => {
			...
			const childDefaultPrompt = async (): Promise<string> => {
				...
				return promptText(ctx.getSystemPrompt?.()) || "(pi default — could not be resolved from this pi installation)";
			};
			const stack = modelStack();
